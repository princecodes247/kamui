...
describe("EventBus", () => {
  ...
  it("should allow removing listeners", () => {
    const listener = vi.fn((data: {arg: string, form: boolean}) => {});
    const event = createEvent(listener);
  ...
  it("should handle multiple listeners for the same event", () => {
    ...
    const listener3 = vi.fn((data: {arg: boolean, arg2: string, arg3?: string}) => {
      return data;
    });
  ...
  it("should handle events with multiple arguments", () => {
    const listener = vi.fn((data: {orderId: string; status: "shipped" | "in_transit", isPaid: boolean}) => {});
    const event = createEvent(listener);
    const bus = createEventBus({"order.status": event});

    const mockRes = {orderId: "order123", status: "shipped", isPaid: true} as const
